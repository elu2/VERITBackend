...
def nx_dfs(query, depth, thickness_bound):
    ...
    full_df.drop(full_df[full_df['thickness'] < thickness_bound].index, inplace = True)
    ...
    for i in range(depth-1):
        ...
        for row in next_rows:
            ...
            row_df=df_from_nx(T)
            row_df["depth"]=i+2
            row_df.drop(row_df[row_df['thickness'] < thickness_bound].index, inplace = True)
    ...
    #TODO: Get reverse edges
        
    return full_df

def query(query, depth, thickness_bound):
    
    full_df=nx_dfs(query, depth, thickness_bound)
    ...
    ev_df=pd.read_csv("evidence.csv")
...
query(query,depth,thickness_bound)
