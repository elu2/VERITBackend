...
def preproc(df):
    ...
    offenders = [":", "{", "[", "::uberon:UBERON:",
                 "::cl:CL:", "ice::uniprot:P41944", "ass::go:GO:0016049"]
    comb_ser = df["OUTPUT"] + df["CONTROLLER"]

    drop_rows = comb_ser.str.contains(
        "|".join(offenders), regex=False) == False
    drop_rows = (df["OUTPUT"].str.contains(
        "(.*::.*::.*)|(.*::(\..){0,1}$)") == False) * drop_rows
    drop_rows = (df["OUTPUT"].str.contains(":") == True) * drop_rows
    drop_rows = (df["CONTROLLER"].str.contains(
        "(.*::.*::.*)|(.*::(\..){0,1}$)") == False) * drop_rows
    ...
    output_ids = df["OUTPUT"].str.split("::", expand=True, regex=False).rename(
        columns={0: "OUTPUT NAME", 1: "OUTPUT ID"})
    output_ids["OUTPUT ID"] = output_ids["OUTPUT ID"].str.split(
        ":", regex=False).apply(lambda x: f"{x[-2]}:{x[-1]}")
    anom_rows = output_ids["OUTPUT ID"].str.contains(".", regex=False)
    output_ids["OUTPUT ID"][anom_rows] = output_ids["OUTPUT ID"][anom_rows].str.split(
        ".", regex=False).apply(lambda x: x[0])

    contro_ids = df["CONTROLLER"].str.split("::", expand=True, regex=False).rename(
        columns={0: "CONTROLLER NAME", 1: "CONTROLLER ID"})
    contro_ids["CONTROLLER ID"] = contro_ids["CONTROLLER ID"].str.split(
        ":", regex=False).apply(lambda x: f"{x[-2]}:{x[-1]}")
    anom_rows = contro_ids["CONTROLLER ID"].str.contains(".", regex=False)
    contro_ids["CONTROLLER ID"][anom_rows] = contro_ids["CONTROLLER ID"][anom_rows].str.split(
        ".", regex=False).apply(lambda x: x[0])
...
def concat_ev(evidence_df, drop_degree=1):
    ...
    count_df = evidence_df.groupby(["OUTPUT ID", "CONTROLLER ID"]).size(
    ).reset_index().rename(columns={0: "TOT COUNTER"})
    count_df = count_df[count_df["TOT COUNTER"]
                        > drop_degree].reset_index(drop=True)
    evidence_df = count_df.merge(
        evidence_df, on=["OUTPUT ID", "CONTROLLER ID"]).drop("TOT COUNTER", axis=1)

    # Add separators to labels and PMCIDs for later
    evidence_df["EVENT LABEL"] = evidence_df["EVENT LABEL"].apply(
        lambda x: f"|{x}|")
    ...
    evidence_df["EVIDENCE"] = evidence_df[["EVIDENCE",
                                           "SEEN IN", "EVENT LABEL"]].agg(" ".join, axis=1)

    # Join all processed evidence by interaction pair
    evidence_df = evidence_df.groupby(["OUTPUT ID", "CONTROLLER ID"])[
        "EVIDENCE"].apply("%%".join).reset_index()
...
def get_edges(df, drop_degree=1):
    ...
    counted = df.groupby(
        by=["OUTPUT ID", "CONTROLLER ID", "EVENT LABEL"]).size()
    counted = pd.DataFrame(counted).reset_index().rename(
        columns={0: "COUNTER"})
    df = df.merge(counted, on=["OUTPUT ID", "CONTROLLER ID", "EVENT LABEL"])

    # Sum together all occurences of interaction
    inter_tts = counted.groupby(["OUTPUT ID", "CONTROLLER ID"])[
        "COUNTER"].sum().reset_index()

    event_pivot = counted.pivot(
        index=["OUTPUT ID", "CONTROLLER ID"], columns="EVENT LABEL", values="COUNTER").reset_index()

    event_props = inter_tts.merge(
        event_pivot, on=["OUTPUT ID", "CONTROLLER ID"]).fillna(0)

    event_props["CONF"] = (event_props["Activation (Positive)"] -
                           event_props["Activation (Negative)"]) / event_props["COUNTER"]

    event_props = event_props.drop(
        columns=["Activation (Positive)", "Activation (Negative)", "Inconclusive"])
...
def pagerank_nodes(nodes, edges):
    G = nx.from_pandas_edgelist(
        edges, "source", "target", edge_attr="thickness")
...
if __name__ == "__main__":
    ...
    new_full_df = pd.concat([new_act_df, new_nc_df], ignore_index=True)
    del new_act_df
    del new_nc_df
    ...
    if os.path.exists("./AllActNC.csv"):
        ...
        full_df = pd.concat([old_act_nc_df, new_full_df], ignore_index=True)
        del old_act_nc_df
        del new_full_df
    else:
        full_df = new_full_df
    ...
    evidence = full_df[["OUTPUT ID", "CONTROLLER ID",
                        "EVENT LABEL", "EVIDENCE", "SEEN IN"]]
